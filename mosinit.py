'''
Current clamp demo using Allen Brain's model 472304676.
...
from neuron472304676 import Neuron472304676
...
def demo(iapp):
    ...
    h.celsius = 34.0
    cell = Neuron472304676(name='neuron')
    ...
    vbox.intercept(0)
    vbox.map('Allen Institute model 472304676; v includes junction potential', 10, 10, 600, 400)

    # procedure for doing and plotting each simulation
    def do_current_clamp_experiment(amp, color):   
        ic.amp = amp / 1000.
        
        h.finitialize(-85.9110921224)
        h.fcurrent()
